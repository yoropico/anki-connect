...
class AnkiConnect:
    ...
    @util.api()
    def getDeckLearningStats(self, decks=None):
        collection = self.collection()

        if decks is None:
            decks = [x.name for x in self.decks().all_names_and_ids()]

        result = {}
        for deck_name in decks:
            # 하위 덱 포함 검색
            card_ids = collection.find_cards(f'"deck:{deck_name}"')
            if not card_ids:
                continue
            ...
            for cid in card_ids:
                ...
                try:
                    stats = collection.card_stats_data(cid)
                    if stats.HasField('fsrs_retrievability'):
                        rs.append(stats.fsrs_retrievability)
                    if stats.HasField('memory_state'):
                        ss.append(stats.memory_state.stability)
                        ds.append(stats.memory_state.difficulty)
                except:
                    pass
            ...
            deck_stats = {
                ...
                'avg_difficulty': round(sum(ds) / len(ds), 2) if ds else None,
                'avg_stability': round(sum(ss) / len(ss), 1) if ss else None,
                'avg_retrievability': round(sum(rs) / len(rs), 4) if rs else None,
                'at_risk_count': sum(1 for r in rs if r < 0.7),
                'review_rate': round(mature / active * 100, 1) if active > 0 else 0,
            }
    ...
    @util.api()
    def createFilteredDeck(self, name, searchQuery, limit=100, order=0, reschedule=True):
        collection = self.collection()

        # 기존 동명 필터 덱이 있으면 비우고 재사용
        existing_did = collection.decks.id_for_name(name)
        if existing_did:
            existing_deck = collection.decks.get(existing_did)
            if existing_deck and existing_deck.get('dyn', 0):
                collection.sched.empty_filtered_deck(existing_did)
                collection.decks.remove([existing_did])

        # 필터 덱 생성
        did = collection.decks.new_filtered(name)
        deck = collection.decks.get(did)

        # 검색 조건 설정
        deck['terms'] = [[searchQuery, limit, order]]
        deck['resched'] = reschedule
        collection.decks.save(deck)

        # 카드 채우기
        collection.sched.rebuild_filtered_deck(did)

        # 결과 카드 수 확인
        card_ids = collection.find_cards(f'"deck:{name}"')
    ...
    @util.api()
    def emptyFilteredDeck(self, deck):
        collection = self.collection()
        did = collection.decks.id_for_name(deck)
        if did is None:
            raise Exception(f'deck not found: {deck}')
        deck_obj = collection.decks.get(did)
        if not deck_obj.get('dyn', 0):
            raise Exception(f'not a filtered deck: {deck}')
        collection.sched.empty_filtered_deck(did)


    @util.api()
    def rebuildFilteredDeck(self, deck):
        collection = self.collection()
        did = collection.decks.id_for_name(deck)
        if did is None:
            raise Exception(f'deck not found: {deck}')
        ...
        if not deck_obj.get('dyn', 0):
            raise Exception(f'not a filtered deck: {deck}')
        collection.sched.rebuild_filtered_deck(did)
        card_ids = collection.find_cards(f'"deck:{deck}"')
        return {'cards_count': len(card_ids)}
